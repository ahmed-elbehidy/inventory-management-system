...
def select_data(event,empid_entry, name_entry, email_entry, gender_combobox,
                dob_date_entry, contact_entry, employeement_type_combobox,
...
def add_employee(empid, name, email, gender, dob, contact, employeement_type, education, work_shift, address, doj,
                 salary, usertype, password):
    if (empid == '' or name == '' or email == '' or gender == 'Select Gender' or employeement_type == 'Select Type' or
        ...
        try:
            ...
            cursor.execute('INSERT INTO employee_data VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)',(empid, name, email, gender, dob, contact, employeement_type, education, work_shift, address, doj,
                                                                                                                salary, usertype, password))
            connection.commit()
            treeview_data()
...
def update_employee(empid, name, email, gender, dob, contact, employeement_type, education, work_shift, address, doj,
                    salary, usertype, password):
    ...
    if not selected:
        ...
        try:
            ...
            address = address.strip()
            new_data = (name, email, gender, dob, contact, employeement_type, education, work_shift, address, doj,
                        salary, usertype, password)

            if current_data == new_data:
                messagebox.showinfo('Information','No changes detected')
                return
            cursor.execute('UPDATE employee_data SET name=%s, email=%s, gender=%s, dob=%s, contact=%s, employeement_type=%s, education=%s,'
                           'work_shift=%s, address=%s, doj=%s, salary=%s, usertype=%s, password=%s WHERE empid=%s',
                           (name, email, gender, dob, contact, employeement_type, education, work_shift, address, doj,
                            salary, usertype, password, empid))
            connection.commit()
            treeview_data()
...
def employee_form(window):
    ...
    search_combobox = ttk.Combobox(
        search_frame,
        values=('EmpId', 'Name', 'Email', 'Gender', 'Employeement Type', 'Education', 'Work shift'),
    ...
    employee_treeview = ttk.Treeview(
        top_frame,
        columns=('empid', 'name', 'email', 'gender', 'dob', 'contact', 'employeement_type',
    ...
    employee_treeview.heading('employeement_type', text='Employeement Type')
    ...
    employee_treeview.column('employeement_type', width=120)
    ...
    employeement_type_combobox = ttk.Combobox(
        detail_frame,
    ...
    education_options = ['BS.Tech', 'B.Com', 'M.Tech', 'M.Com', 'B.Sc', 'M.Sc', 'BBA', 'MBA', 'LLB', 'LLM', 'B.Arch','M.Arch']
    ...
    add_button = Button(
        ...
        command=lambda: add_employee(empid_entry.get(), name_entry.get(), email_entry.get(), gender_combobox.get(),
                         dob_date_entry.get(), contact_entry.get(), employeement_type_combobox.get(),
